...
function intp_result = func_itp()
    intp_result.interparc = @interparc;
    intp_result.polyfit_inter = @polyfit_inter;
    intp_result.interpft = @interpft;
end

% Fourier interpolation on nonuniform points 
function [xe,xse,xsse,relres,x_h,k] = interpft(x,t,Nk,N)
%   x - sample values
%   t - sample points
%   Nk - number of Fourier modes
%   N - number of grid points

%   xe - interpolation result
%   xse - first derivative result
%   relres - standardized error
%   x_h: fourier conefficient
%   k: wave number

    [~,id] = sort(t);
    t = t(id); x = x(id);
    L = t(end) - t(1); % domain size
    
    if mod(Nk,2) == 0
        kp = 0:(Nk/2);
        km = (-Nk/2+1):-1;
        k = (2*pi/L)*[kp km]; % list of wave numbers
    else
        kp = 0:(Nk-1)/2;
        km = -(Nk-1)/2:-1;
        k = (2*pi/L)*[kp km]; % list of wave numbers
    ...
    S = (1/Nk)*exp(1i*t*k); % Fourier transform matrix
    x_h = S\x; % compute Fourier coefficients

    [~,~,relres] = lsqr(S,x);

    xe_h = [x_h(kp+1);zeros(N-Nk,1);x_h(kp(end)-fliplr(km)+1)]*(N/Nk);
    xe = real(ifft(xe_h));

    % calculate the first order derivative
    xs_h = 1i.*k'.*x_h;
    xse_h = [xs_h(kp+1);zeros(N-Nk,1);xs_h(kp(end)-fliplr(km)+1)]*(N/Nk);
    xse = real(ifft(xse_h));

    % calculate the second order derivative
    xss_h = -(k').^2.*x_h;
    xsse_h = [xss_h(kp+1);zeros(N-Nk,1);xss_h(kp(end)-fliplr(km)+1)]*(N/Nk);
    xsse = real(ifft(xsse_h));
end

% equidistributed points finding on a curve
% using method pchip, spline, csape
